...
class AppLoggingService implements LoggingService {
  ...
  static Future<void> _loadConfiguration() async {
    try {
      ...
      // Parse auto flush option
      _autoFlush = config['auto_flush'] as bool? ?? false;
  ...
  void debug(String message, [dynamic error, StackTrace? stackTrace]) {
    ...
    if (_autoFlush) _fireAndForget(_fileOutput?.flushAndWait());
  }
  ...
  // Run a future without awaiting it and log errors
  static void _fireAndForget(Future<void>? fut) {
    if (fut == null) return;
    fut.catchError((e) {
      debugPrint('Background flush error: $e');
    });
  }
...
class AsyncBufferedFileOutput extends LogOutput {
  ...
  // Regex to remove ANSI escape sequences (including common SGR color codes)
  ...
  String _sanitize(String s) => s.replaceAll(_ansiRegex, '');
  ...
  void output(OutputEvent event) {
    try {
      ...
      final sanitized = _sanitize(logEntry);
  ...
  void outputRaw(String message) {
    try {
      final timestamp = DateTime.now().toIso8601String();
      final sanitized = _sanitize('$timestamp: $message');
      _buffer.add(sanitized);
